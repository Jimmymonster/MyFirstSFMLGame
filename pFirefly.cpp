#include "pFirefly.h"
pFirefly::pFirefly(unsigned int count)
	:particles(count), lifeTime(sf::seconds(2.5f)), fireflies(count)
{
...
void pFirefly::resetParticle(std::size_t i)
{
	...
	if (position.x >= 400 && position.x <= 1200 && position.y<=700) position.x %= 400;
